...
namespace Identity.JwtToken.Extensions
{
    ...
    {
        public static string GenerateJwtToken(this IdentityUser user, IConfiguration configuration,
            Models.TokenOptions options = null)
        {
            var claims = new[]
            {
            ...
            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(configuration[options?.ConfKeyPath ?? "Jwt:Key"]));
